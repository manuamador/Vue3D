    ...
    function fillToolTip(features, layer, options) {
        ...
        for (var p = 0; p < features.length; p++) {
            ...
            if (geometry.properties) {
								addcontent=( geometry.properties.Description || "<span style='font-size:16px'><b>"+geometry.properties.nom+'</b></span>' || "<span style='font-size:16px'><b>"+geometry.properties.rec+'</b></span>'+"<i><br><span style='font-size:12px'>"+geometry.properties.toponyme+'</span></i>' || layer.name || '');
								if (!(content.includes(addcontent))){ 
								//content += '<p>';
								//content += '<span style="color: ' + fill + ';font-size:16px; -webkit-text-stroke: ' + stroke + '">';
								//content += symb + ' ';
								//content += '</span>';
                                content += ( geometry.properties.Description || "<span style='font-size:16px'><b>"+geometry.properties.nom+'</b></span>' || "<span style='font-size:16px'><b>"+geometry.properties.rec+'</b></span>'+"<i><br><span style='font-size:12px'>"+geometry.properties.toponyme+'</span></i>' || layer.name || '');}
            }
